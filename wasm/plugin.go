...
func (ctx *httpContext) OnHttpRequestHeaders(numHeaders int, endOfStream bool) types.Action {
	...
	// Try to get source pod from Envoy metadata.
	if pod, err := proxywasm.GetProperty([]string{"node", "metadata", "NAME"}); err == nil {
...
func (ctx *httpContext) OnHttpRequestBody(bodySize int, endOfStream bool) types.Action {
	...
	readSize := bodySize
	if readSize > ctx.config.MaxBodySize {
		readSize = ctx.config.MaxBodySize
	}
	...
	if readSize > 0 {
		...
		if err != nil {
			proxywasm.LogWarnf("failed to get request body: %v", err)
		}
	}

	ctx.dispatchToCollector(body)
	return types.ActionContinue
}
