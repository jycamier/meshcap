...
| Value | Default | Description |
|-------|---------|-------------|
| `otel.endpoint` | `""` | OTLP gRPC endpoint (e.g. `http://otel-collector.observability:4317`) |

When set, the collector exports distributed traces via OTLP gRPC. When empty, the OTEL SDK operates as a noop (zero overhead). No other configuration is needed — the standard `OTEL_*` environment variables are supported natively by the SDK.
...
You can also pass additional OTEL environment variables via `podAnnotations` or by extending the ConfigMap (e.g. `OTEL_RESOURCE_ATTRIBUTES`, `OTEL_TRACES_SAMPLER`).
